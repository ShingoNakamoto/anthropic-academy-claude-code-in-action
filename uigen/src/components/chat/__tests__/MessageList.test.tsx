...
test("MessageList shows empty state when no messages", () => {
  ...
  expect(
    screen.getByText("会話を始めてReactコンポーネントを生成しましょう")
  ).toBeDefined();
  expect(
    screen.getByText("ボタン、フォーム、カードなどの作成をお手伝いします")
  ).toBeDefined();
});
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByText("ファイルを操作:")).toBeDefined();
});
...
test("MessageList renders reasoning parts", () => {
  ...
  expect(screen.getByText("推論")).toBeDefined();
  expect(
